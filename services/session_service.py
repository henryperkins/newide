...
class SessionService:
    ...
    async def create_session(
        ...
        """
        # Create a span for session creation
        with sentry_sdk.start_span(op="session.create", description="Create New Session") as transaction:
    ...
    async def validate_session(
        ...
        """
        Validate a session by ID, checking if it exists, is not expired, and belongs to the specified user.
        
        Args:
            session_id: The session ID to validate
            db_session: Database session
            user_id: Optional user ID to verify ownership
            require_valid: If True, raise exceptions for invalid sessions
        ...
        with sentry_sdk.start_span(op="session.validate", description="Validate Session") as span:
            ...
            try:
                session_uuid = session_id if isinstance(session_id, uuid.UUID) else uuid.UUID(session_id, version=4)
                span.set_data("session_id", str(session_uuid))
            ...
            with sentry_sdk.start_span(op="session.check_rate_limit", description="Check Rate Limits") as rate_span:
                try:
                    ...
                    # Re-raise rate limit exception if required
                    if require_valid:
                        raise rate_error
                    return None
            
            # Check user ownership if user_id is provided
            if user_id and session.session_metadata and 'owner_id' in session.session_metadata:
                owner_id = session.session_metadata.get('owner_id')
                if owner_id != str(user_id):
                    if require_valid:
                        ...
                        add_breadcrumb(
                            ...
                            level="warning",
                            data={"session_id": str(session_uuid), "expected_user": str(user_id), "actual_user": owner_id}
                        )
            ...
            # Associate user ID with session if available in metadata
            if session.session_metadata and 'owner_id' in session.session_metadata:
                owner_id = session.session_metadata.get('owner_id')
