...
class AzureFileService:
    ...
    async def get_vector_store_file_status(
        self, vector_store_id: str, file_id: str
    ) -> str:
    ...
    async def wait_for_file_processing(
        self, vector_store_id: str, file_id: str, timeout: int = 300
    ) -> bool:
        ...
        while (datetime.now() - start_time).total_seconds() < timeout_seconds:
            try:
                status = await self.get_vector_store_file_status(
    ...
    async def create_file_batch(self, vector_store_id: str, file_ids: List[str]) -> str:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                result = await response.json()
                logger.info(
                    f"Created file batch with {len(file_ids)} files in vector store {vector_store_id}"
                )
                return result["id"]
    ...
    async def get_vector_store_for_session(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"id": vector_store_id, "name": f"session-{session_id}"}
        except Exception as e:
