...
async def flag_user_for_password_reset(db_session: AsyncSession, user_id: str) -> None:
    ...
    try:
        ...
        if user:
            ...
            logger.info(
                f"Flagged user {user_id} for password reset due to invalid hash format"
            )
...
async def notify_admin_about_hash_issue(
    ...
    try:
        ...
        sent = await email_service.send_admin_notification(
            subject=subject, message=message, event_data=event_data
        )
...
async def generate_password_reset_token(
    user_id: str, expires_in_minutes: int = 60
) -> str:
    """
...
async def initiate_password_reset(db_session: AsyncSession, email: str) -> bool:
    ...
    try:
        ...
        sent = await email_service.send_password_reset_email(
            to_email=email, reset_token=token, user_id=str(user.id)
        )
