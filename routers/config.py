...
# Assuming session_utils exists and contains necessary functions.  This was missing.
from session_utils import SessionManager  # Import SessionManager
    ...
    if session_id:
        try:
            async with AsyncSessionLocal() as db:
                ...
                from models import Session  # Assuming you have a Session model
...
@router.get(
    "/models/{model_id}", response_model=ModelConfigModel
)  # Specify response model
async def get_model(model_id: str, db_session: AsyncSession = Depends(get_db_session)):
    """Get a specific model configuration.  Another key endpoint."""
    try:
...
async def create_model(
    ...
    try:
        ...
        if not model:
            ...
            if not model:  # If no template found
                raise HTTPException(
                    status_code=400,
                    detail=f"No template found for model {actual_model_id}",
                )
        ...
        try:
            ...
            validated_model = ModelConfigModel(**model)
...
@router.put(
    "/models/{model_id}", response_model=Dict[str, str]
)  # Specify response model
async def update_model(
    ...
    """Update an existing model configuration. Another key endpoint."""
    try:
        # Get client pool
        client_pool = await get_client_pool(db_session)
        ...
        # Update model
        success = await client_pool.add_or_update_model(
            model_id, model.model_dump(), db_session  # Use model_dump
        )
...
async def delete_model(
    ...
    """Delete a model configuration.  Another key endpoint."""
    try:
        # Get client pool
        client_pool = await get_client_pool(db_session)
...
@router.post("/models/switch", response_model=Dict[str, Any])  # Specify response model
async def switch_model(
    request: ModelSwitchRequest,
    db_session: AsyncSession = Depends(get_db_session),
):
    """Switch the active model for the current session.  A key endpoint."""
    try:
...
@router.post(
    "/models/switch_model/{model_id}", response_model=Dict[str, Any]
...
async def switch_model_path(
    ...
    try:
        ...
        success = await SessionManager.update_session_model(
            str(session_uuid), model_id, db_session  # Pass session_uuid as string
        )
...
async def get_current_model(
    ...
    """
    Retrieves the currently selected model for the session. This is a dedicated endpoint
    for /api/config/current-model.
    """
    session_id = request.cookies.get("session_id")

    if not session_id:
        return {"currentModel": None}  # No session, no current model
    ...
    try:
        ...
        logger.error(f"Error fetching current model for session {session_id}: {e}")
        raise HTTPException(status_code=500, detail="Error retrieving current model")
